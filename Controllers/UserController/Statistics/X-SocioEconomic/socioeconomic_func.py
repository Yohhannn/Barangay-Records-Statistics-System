...
class socioeconomic_func(BaseFileController):
    def __init__(self, login_window, emp_first_name, stack):
        super().__init__(login_window, emp_first_name)
        self.stack = stack
        self.stat_socioeconomic_screen = self.load_ui("Views/MainPages/StatisticPages/socioeconomic.ui")
        self._setup_date()
        self._setup_time()
        self.setup_socioeconomic_ui()
        self.center_on_screen()

    def setup_socioeconomic_ui(self):
        """Setup the SocioEconomic Views layout."""
        self.setFixedSize(1350, 850)
        self.setWindowTitle("MaPro: SocioEconomic")
        self.setWindowIcon(QIcon("Resources/AppIcons/appicon_active_u.ico"))

    # Set images and icons
        self.stat_socioeconomic_screen.btn_returnToStatisticsPage.setIcon(QIcon('Resources/FuncIcons/img_return.png'))

        # Return Button
        self.stat_socioeconomic_screen.btn_returnToStatisticsPage.clicked.connect(self.goto_statistics_panel)

    def _setup_time(self):
        self.timer = QTimer(self.stat_socioeconomic_screen)
    ...
    def _setup_date(self):
        ...
        self.stat_socioeconomic_screen.text_date.setText(f"{formatted_date} - {day_of_week}")

        # Update time label
        formatted_time = current_datetime.toString("h:mm:ss AP")
        self.stat_socioeconomic_screen.text_time.setText(formatted_time)
