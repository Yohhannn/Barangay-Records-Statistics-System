...
from Functions.dashboard_functions.citizen_func import citizen_func
...
class dashboard_func(QMainWindow):
    ...
    def setup_citizen_panel_ui(self):
        ctz = citizen_func(self, self.emp_first_name)
        ...
        if not self.citizen_panel_initialized:  # Ensure connections are made only once
            ...
            # self.citizen_panel_screen.profileList_buttonFilter.clicked.connect(ctz.show_filter_popup)
            # self.citizen_panel_screen.profileList_buttonRegister.clicked.connect(ctz.show_register_citizen_part_01_popup())
